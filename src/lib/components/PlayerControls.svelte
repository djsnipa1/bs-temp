      ...
      .add(
        scaleCircle(circle1, {
          ...
          begin: function (anim) {
            ...
            console.log('scale anim');
          },
         // offset: -900 // Starts at 1000ms
        }),
        0
      )
      .add(
        opacityCircle(circle1, {
          duration: 1200,
          end: function (anim) {
            // circle1.style.visibility = 'visible';
            circle1.classList.add = 'hidden';
            console.log('opacity anim');
          },
//          offset: 0 // Starts at 1000ms
        }),
        0
      )
      .add(killOpacity(circle1), '-=900')
      .add(visible(circle2), 1500)
      .add(scaleCircle(circle2, { scale: [0, 8], duration: 1200 }))
      .add(opacityCircle(circle2, { duration: 800 }), 1400);
    /*    .add(
        {
          targets: div3,
          scale: 8,
...
>
  ...
  ></div>
  <div
    class="border-6 absolute hidden rounded-full border-blue-500 p-4 text-white"
    bind:this={circle2}
  ></div>
  <div
    class="absolute hidden rounded-full border border-white p-4 text-white"
    bind:this={circle3}
  ></div>
